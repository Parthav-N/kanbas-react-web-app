...
export default function Assignments() {
    ...
    const filteredAssignments = assignments.filter((assignment: Assignment) => assignment.course === cid);
        ...
        <div id="wd-assignments">
            <div className="row align-items-center">
                ...
                <div className="col-auto d-flex justify-content-end">
                    ...
                    <button id="wd-add-assignment" className="btn" onClick={handleAddAssignment}>+ Assignment</button>
                </div>
            </div>
            <br />
                    ...
                    <li key={assignment._id} className="wd-assignment-list-item d-flex align-items-center mt-3">
                        ...
                        <div className="assignment-content text-start flex-grow-1">
                            <h3 className="assignment-title mb-0">
                                <Link to={`/Kanbas/Courses/${cid}/Assignments/${assignment._id}`} style={{ textDecoration: 'none', color: 'inherit' }}>
                                    {assignment.title}
                                </Link>
                            </h3>
                            <div className="assignment-details">
                                <span className="text-danger">Multiple Modules</span> | <b>Due</b> {formatDate(assignment.dueDate)} | {assignment.points} pts
                            </div>
                        </div>
                        <div className="d-flex">
                            <LessonControlButtons />
                            <button className="btn btn-danger ms-2" onClick={() => handleDeleteAssignment(assignment)}>
                                <BsTrash />
                            </button>
                        </div>
                    </li>
            ...
            {showConfirmDialog && (
                <div className="confirm-dialog">
                    ...
                    <button className="btn btn-danger ms-2" onClick={confirmDelete}>Delete</button>
                </div>
            )}
        </div>
    );
